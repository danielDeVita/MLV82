...
export class EnemyDodgingPlane extends EnemyPlane {
    constructor(game, speedBoost = 0) {
        ...
        this.scoreValue = 50; // Increased score for added threat
        this.color = 'orange';
        this.enemyType = 'air';

        // --- Sine Wave Properties (Wide Amplitude, FASTER Frequency) ---
        this.amplitude = 60 + Math.random() * 40; // New Range: 60px to 100px (Much narrower)
        // --- INCREASED Frequency ---
        this.frequency = 0.08 + Math.random() * 0.04; // Range: 0.08 to 0.12 (Much faster oscillation)
        this.angle = Math.random() * Math.PI * 2;
        this.initialY = undefined;

        // --- Reactive Dodge Properties --- (Keep these)
        this.dodgeCooldown = 600 + Math.random() * 600;
        this.dodgeCheckTimer = Math.random() * this.dodgeCooldown;
        this.isReacting = false;
        this.reactionDuration = 700;
        this.reactionTimer = 0;
        this.reactionStrength = 120;
        this.reactionDirection = 0;

        // --- Smoothing Properties --- (Keep these)
        this.currentReactionOffset = 0;
        this.targetReactionOffset = 0;
        this.baseReactionLerpSpeed = 0.1;
        ...
        this.shootInterval = 1800 + Math.random() * 1200; // Range: 1.8s to 3.0s
    ...
    update(deltaTime) {
        ...
        if (this.x + this.width < 0) { this.markedForDeletion = true; return; }

        // --- Capture initialY safely ONCE ---
        if (this.initialY === undefined || isNaN(this.initialY)) {
            this.initialY = this.y;
            if (isNaN(this.initialY)) { this.initialY = this.game.height / 2; }
        }
        ...
        if (isNaN(this.y)) { console.error("!!! this.y became NaN! Resetting."); this.y = this.game.height / 2; }
    ...
    attemptReaction() { /* ... same as before ... */
    ...
    draw(context) {
        ...
        else { context.fillStyle = this.color; } // Use orange color
