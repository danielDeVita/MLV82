...
export class EnemyTrackingShip extends EnemyShip {
    ...
    update(deltaTime) {
        ...
        if (!this.markedForDeletion && this.launchTimer >= this.launchInterval && this.x < this.game.width * 0.9) {
            this.launchMissile();
            this.launchTimer = 0;
    ...
    draw(context) {
        ...
        context.fillRect(this.x + this.width * 0.3, this.y - this.height * 0.1, this.width * 0.1, this.height * 0.3); // Silo 1
        context.fillRect(this.x + this.width * 0.6, this.y - this.height * 0.1, this.width * 0.1, this.height * 0.3); // Silo 2
