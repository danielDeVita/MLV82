...
export class Boss3 extends Enemy {
  ...
  createWeakPoints() {
    ...
    weakPointData.forEach((data) => {
      ...
      if (data.type === "hangar") {
        this.hangarSpawnTimers.push({
          timer: randomInt(4000, 7000),
          interval: randomInt(8000, 12000),
        ...
        this.upSpreadGunTimers.push({
          ...
          interval:
            this.upSpreadGunBaseInterval +
            Math.random() * this.upSpreadGunRandomInterval,
          wpIndex: currentIndex,
        });
      }
    });
  ...
  spawnPlaneFromHangar(hangarIndex) {
    const hangarTimerData = this.hangarSpawnTimers.find(
      (t) => t.wpIndex === hangarIndex
    );
    ...
    hangarTimerData.timer =
      hangarTimerData.interval + Math.random() * 3000 - 1500;
  }
  ...
  fireLowArtillery() {
    ...
    const launchX =
      this.x + this.width * 0.5 + (Math.random() - 0.5) * this.width * 0.4;
  ...
  fireLowSpreadShot() {
    ...
    angles.forEach((angle) => {
      ...
      const speedY = Math.sin(angle) * bulletSpeed;
      this.game.addEnemyProjectile(
        new EnemyBullet(this.game, launchX, bulletYCenter, speedX, speedY)
      );
    });
  }
  // --- End Attack Methods ---
  ...
  update(deltaTime) {
    ...
    for (const hst of this.hangarSpawnTimers) {
      if (
        hst.wpIndex === undefined ||
        hst.wpIndex < 0 ||
        hst.wpIndex >= this.weakPoints.length
      ) {
        continue;
      }
    ...
    for (const ust of this.upSpreadGunTimers) {
      if (
        ust.wpIndex === undefined ||
        ust.wpIndex < 0 ||
        ust.wpIndex >= this.weakPoints.length
      ) {
        continue;
      }
    ...
    if (this.fixedArtilleryTimer <= 0) {
      ...
      this.fixedArtilleryTimer =
        this.fixedArtilleryInterval + Math.random() * 500 - 250;
    }
    ...
    if (this.lowMissileTimer <= 0) {
      this.fireLowMissile();
      this.lowMissileTimer =
        this.lowMissileInterval + Math.random() * 2000 - 1000;
    }
    this.lowArtilleryTimer -= safeDeltaTime;
    if (this.lowArtilleryTimer <= 0) {
      this.fireLowArtillery();
      this.lowArtilleryTimer =
        this.lowArtilleryInterval + Math.random() * 1500 - 750;
    }
    ...
    if (this.powerUpTimer >= this.powerUpInterval) {
      ...
      this.powerUpInterval =
        this.powerUpBaseInterval + Math.random() * this.powerUpRandomInterval;
      const spawnX =
        Math.random() * (this.game.width * 0.8) + this.game.width * 0.1;
    ...
    for (const wp of this.weakPoints) {
      if (wp.isActive && checkCollision(projectile, wp)) {
        ...
        const effectiveDamage =
          damage *
          (isBomb && ["hangar", "radar"].includes(wp.type) ? 2.0 : 1.0);
      ...
      this.weakPoints.forEach((wp, index) => {
        if (wp && typeof wp.draw === "function") {
          ...
          try {
            ...
            console.error(
              `ERROR drawing Weak Point type ${wp.type} at index ${index}:`,
              wpDrawError
            );
          } finally {
            context.restore();
          ...
          console.warn(
            `Attempted to draw invalid weak point at index ${index}:`,
            wp
          );
        }
      });
