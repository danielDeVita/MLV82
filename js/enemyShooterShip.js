...
export class EnemyShooterShip extends EnemyShip {
    ...
    update(deltaTime) {
        ...
        if (!this.markedForDeletion && this.shootTimer >= this.shootInterval && this.x < this.game.width * 0.95) {
            this.shoot();
    ...
    draw(context) {
        ...
        context.fillRect(this.x + this.width * 0.4, this.y - this.height * 0.15, this.width * 0.2, this.height * 0.3); // Taller turret
